...
def plot_from_csvs(csvs, suffix='pred_x'):
    ...
    for i, csv in enumerate(csvs):
        ...
        if i == 0:
            ax_map.plot(T_glob_gt[:, 0, 3], T_glob_gt[:, 1, 3], alpha=0.8, linewidth=1, label='Ground Truth',
                        color='black')
        ...
        if test_nr not in loss_exclude:
            ax_loss.plot(range(len(losses)), losses, colors[test_nr], alpha=0.8, linewidth=1,
                         label='Test {}'.format(test_lbl),
                         )
